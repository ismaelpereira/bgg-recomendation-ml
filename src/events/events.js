...
export default class Events {
   ...
   static onBoardgameAdded(callback) {
      document.addEventListener(events.boardgameAdded, (event) => {
         return callback(event.detail);
      });
   }
   static dispatchBoardgameAdded(data) {
      const event = new CustomEvent(events.boardgameAdded, {
         detail: data,
      });
      document.dispatchEvent(event);
   }

   static onBoardgameRemoved(callback) {
      document.addEventListener(events.boardgameRemoved, (event) => {
         return callback(event.detail);
      });
   }

   static dispatchEventBoardgameRemoved(data) {
      const event = new CustomEvent(events.boardgameRemoved, {
         detail: data,
      });
      document.dispatchEvent(event);
   }
